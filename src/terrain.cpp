...
void Terrain::GenerateVertices() {
  ...
  for (int z = 0; z < res_z_; z++) {
    for (int x = 0; x < res_x_; x++) {
      ...
      glm::vec3 normal =
          glm::vec3(heightL - heightR, 0.001f, heightD - heightT);
...
float Terrain::GetHeight(int x, int z) {
  ...
  return vertices_[z * res_x_ + x].position.y;
}
